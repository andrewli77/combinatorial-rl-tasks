...
class HierAgent:
    ...
    def __init__(self, obs_space, action_space, model_dir,
                 device=None, num_envs=1, n_skills=3, skill_len=25, h_dim=128):
        obs_space, self.preprocess_obss = utils.get_obss_preprocessor(obs_space)
        self.hi_policy_value_net = HighPolicyValueModel(obs_space, n_skills, h_dim)
        self.lo_policy_value_net = LoPolicyValueModel(obs_space, action_space, n_skills, h_dim)
    ...
    def get_lo_action(self, obs, skill):
        with torch.no_grad():
            preprocessed_obs = self.preprocess_obss([obs], device=self.device)
            preprocessed_obs.skill = skill.to(self.device)
