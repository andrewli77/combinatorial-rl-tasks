...
class BaseAlgo(ABC):
    ...
    def __init__(self, envs, acmodel, device, num_frames_per_proc, discount, lr, gae_lambda, distributional_value,
    ...
    def collect_experiences(self):
        ...
        for i in range(self.num_frames_per_proc):
            ...
            if 'shaped_reward' in info[0]:
                self.rewards[i] = torch.tensor([
                    info[i]['shaped_reward'] for i in range(self.num_procs)
                    ], device=self.device)
            else:
