...
class PPOAlgo(BaseAlgo):
    ...
    def __init__(self, envs, acmodel, device=None, num_frames_per_proc=None, discount=0.99, lr=0.001, gae_lambda=0.95, distributional_value=False,
                 entropy_coef=0.01, value_loss_coef=0.5, max_grad_norm=0.5, recurrence=4,
                 adam_eps=1e-8, clip_eps=0.2, epochs=4, batch_size=256, preprocess_obss=None,
                 reshape_reward=None):
        num_frames_per_proc = num_frames_per_proc or 128

        super().__init__(envs, acmodel, device, num_frames_per_proc, discount, lr, gae_lambda, distributional_value, 
    ...
    def update_parameters(self, exps):
            ...
            for inds in self._get_batches_starting_indexes():
                ...
                for i in range(self.recurrence):
                    ...
                    if self.distributional_value:
                        value_loss = -Normal(value[0], value[1]).log_prob(sb.returnn).mean()
                    else:
                    ...
                    if self.distributional_value:
                        batch_value += value[0].mean().item()
                        batch_value_std += value[1].mean().item()
                    else:
                ...
                batch_value_std /= self.recurrence
                ...
                if self.distributional_value:
                    log_value_stds.append(batch_value_std)
        ...
        if self.distributional_value:
            logs = {
                ...
                "value_std": numpy.mean(log_value_stds),
