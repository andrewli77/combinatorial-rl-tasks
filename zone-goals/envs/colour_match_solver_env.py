...
class ColourMatchSolverEnv(ZoneEnvBase):
    ...
    def solver_get_next_goal(self):
        n_green = 0
        n_blue = 0
        n_red = 0

        def dist_to(i):
            goal_pos = self.zones_pos[i][:2]
        ...
        min_dist = min(dist_to_blue, dist_to_green, dist_to_red)
        candidate_zones = []
        
        for i in range(self.num_cities):
            if self.zones[i] == zone.Blue:
                if dist_to_green == min_dist or dist_to_red == min_dist:
                    candidate_zones.append((dist_to(i), i))

            elif self.zones[i] == zone.Green:
                if dist_to_blue == min_dist or dist_to_red == min_dist:
                    candidate_zones.append((dist_to(i), i))
            elif self.zones[i] == zone.Red:
                if dist_to_blue == min_dist or dist_to_green == min_dist:
                    candidate_zones.append((dist_to(i), i))
            else:
                exit(1)

        candidate_zones.sort()
        return candidate_zones[0][1]
    ...
    def step(self, action):
        ...
        # Case 1: Reached the correct zone
        if self.zones_changed and self.last_visited_zone == self.goal_zone:
            _i['shaped_reward'] = 0.
        # Case 2: Reached incorrect zone (penalized)
        else:
            _dist = self.dist_to_goal()
            _i['shaped_reward'] = self.last_dist_to_goal - _dist
            self.last_dist_to_goal = _dist

            # Reached incorrect zone
            if self.zones_changed:
                _i['shaped_reward'] -= 1. 

        # Check if we need to generate a new goal 
        if (self.zones_changed and self.last_visited_zone == self.goal_zone)  or _d:
    ...
    def set_goal(self, next_goal):
        assert(0 <= next_goal and next_goal < len(self.zones))
    ...
    def get_available_goals(self):
        ...
        available_goals = np.ones(self.num_cities, dtype=bool)
        ...
        for h_index, h_pos in enumerate(self.zones_pos):
            if self.zone_cooldowns[h_index] == 0:
                ...
                if h_dist <= self.zones_size:
                    ...
                    self.last_visited_zone = h_index
