...
def collect_experiences(self):
    for i in range(self.num_frames_per_proc):
        ...
        needs_goal = self.env.needs_goal()
        ...
        for _j in range(len(obs_hi_indices)):
            j = obs_hi_indices[_j]
            available_goals = self.env.available_goals(j)
            dists.logits[_j][~available_goals] = float('-inf') # Mask out unavailable goals
            dist = torch.distributions.Categorical(logits=dists.logits[_j])
            sampled_skill = dist.sample()

            # Set the skill in environment j
            self.env.set_goal(j, sampled_skill.cpu().numpy())
            self.cur_goal[j] = torch.tensor(self.env.get_goal(j))

            # Log high-level experiences
            self.hi_obss[j].append(self.obs[j])
            self.hi_actions[j].append(sampled_skill)
            self.hi_action_masks[j].append(available_goals)
            self.hi_values[j].append(vals[_j])
            self.hi_log_probs[j].append(dist.log_prob(sampled_skill))

        with torch.no_grad():
            preprocessed_obs.goal = self.cur_goal
        ...
        with torch.no_grad():
            ...
            self.lo_rewards[i] = torch.tensor([_i['shaped_reward'] for _i in info], device=self.device)
            self.lo_obss[i] = self.obs
            self.lo_goals[i] = self.cur_goal
            self.obs = obs
    ...
    lo_goals_reshaped = self.lo_goals[:-1].transpose(0,1).reshape((-1, self.goal_dim))
    lo_exps.obs.goal = lo_goals_reshaped
    ...
    hi_exps.action_mask = torch.tensor([self.hi_action_masks[j][i]
                        for j in range(self.num_procs)
                        for i in range(len(self.hi_rewards[j]))], device=self.device)
    ...
    for j in range(self.num_procs):
        ...
        del self.hi_action_masks[j][:n_remove]
...
def update_hi_parameters(self, exps):
    ...
    for _ in range(self.hi_epochs):
        ...
        for inds in self._get_batches_starting_indexes_hi():
            ...
            dist.logits[~sb.action_mask] = float('-inf')
            dist = Categorical(logits=dist.logits)
            entropy = dist.entropy().mean()
