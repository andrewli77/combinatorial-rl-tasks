...
def getLoEnvEncoder(obs_space, goal_dim, h_dim):
    if "goal" in obs_space:
        return ZoneEnvGoalModel(obs_space, goal_dim, h_dim)
...
class ZoneEnvGoalModel(EnvModel):
    def __init__(self, obs_space, goal_dim, h_dim):
        super().__init__(obs_space)
        ...
        self.zone_net_ = nn.Sequential(
            nn.Linear(self.obs_size + goal_dim + zone_size, h_dim),
            nn.ReLU(),
            nn.Linear(h_dim, h_dim),
            nn.ReLU(),
            nn.Linear(h_dim, h_dim),            
        ...
        self.combine_net_ = nn.Linear(self.obs_size + goal_dim + h_dim, h_dim)
